...
export class IipMonthNewComponent implements OnInit {
  ...
  private readonly LINK_DEFAULT: string =
    "/specialized/industry-management/iip/iip-detail";
  ...
  public cssxcnvalue: Array<new_model> = new Array<new_model>();
      ...
      if (this.isExcelFile) {
        ...
        reader.onload = (e: any) => {
          ...
          data.forEach(item => {
            ...
            this.cssxcnvalue.push(datarow)
          });
          this.save(this.timechange, this.cssxcnvalue)
        };
  ...
  public save(month: number, cssxcnvalue: Array<new_model>) {
    this.sctService.CapNhatDuLieuCSSXThang(month, cssxcnvalue).subscribe(
      next => {
        if (next.id == -1) {
  ...
  setSumaryData(data) {
    ...
    console.log(data[0].tri_gia_thang)
  ...
  iip: Array<chartmodel> = new Array<chartmodel>();
